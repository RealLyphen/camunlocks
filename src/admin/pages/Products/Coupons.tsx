...
import { Plus, Search, Edit2, Trash2, Ticket, DollarSign, Percent } from 'lucide-react';
...
const Coupons: React.FC = () => {
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case 'expired': return { bg: 'rgba(251, 146, 60, 0.1)', color: '#fb923c', border: 'rgba(251, 146, 60, 0.25)' };
    ...
    const getTypeStyle = (type: string) => {
        return type === 'fixed'
            ? { bg: 'rgba(99, 102, 241, 0.12)', color: '#818cf8', border: 'rgba(99, 102, 241, 0.25)', icon: <DollarSign size={12} /> }
            : { bg: 'rgba(139, 92, 246, 0.12)', color: '#a78bfa', border: 'rgba(139, 92, 246, 0.25)', icon: <Percent size={12} /> };
    };
    ...
    return (
        <div className="relative">
            ...
            <div className="admin-header">
                ...
                <Link to="/admin/products/coupons/create" className="btn-primary">
            ...
            >
                {[
                    ...
                    { label: 'Active', value: coupons.filter(c => c.status === 'active').length, color: '#4ade80' },
                    { label: 'Total Uses', value: coupons.reduce((sum, c) => sum + c.usageCount, 0), color: '#fbbf24' },
                ].map((stat, i) => (
                    <div key={i} style={{
            ...
            >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#52525b', fontSize: '0.85rem' }}>
                        <Ticket size={16} />
                        <span>{filteredCoupons.length} coupon{filteredCoupons.length !== 1 ? 's' : ''}</span>
                    </div>
                </div>
            </motion.div>
            ...
            >
                <table className="data-table">
                    <thead>
                        <tr>
                            <th style={{ paddingLeft: 24 }}>Coupon Code</th>
                            <th>Type</th>
                            <th>Discount</th>
                            <th>Status</th>
                            <th>Usage</th>
                    ...
                    <tbody>
                        {displayedCoupons.length > 0 ? (
                            displayedCoupons.map((coupon) => {
                                ...
                                const typeStyle = getTypeStyle(coupon.type);
                                return (
                                    <tr key={coupon.id} style={{ transition: 'background 0.2s' }}
                                    ...
                                    >
                                        ...
                                        <td style={{ paddingRight: 24 }}>
                                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                                                <Link
                                                    to={`/admin/products/coupons/edit/${coupon.id}`}
                                                    style={{
                            ...
                            <tr>
                                <td colSpan={7} style={{ padding: '48px 0', textAlign: 'center' }}>
                                    <div style={{
                                        ...
                                        <p>{search ? `No coupons matching "${search}"` : 'No coupons yet. Create your first coupon!'}</p>
                                    </div>
                                </td>
                            </tr>
                        )}
