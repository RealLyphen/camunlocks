...
const EditProduct: React.FC = () => {
    ...
    const [slugManuallyEdited, setSlugManuallyEdited] = useState(true); // true by default for edit
    ...
    /* ─── Pre‑populate from existing product ─── */
    useEffect(() => {
        if (id && products.length > 0) {
            ...
            if (product) {
                ...
                setHideProduct(product.status === 'hidden');
                setFeatured(product.featured || false);
                setCapStock(product.stockCap != null);
                setStockQty(product.stockCap != null ? product.stockCap.toString() : '');
                setPayWhatYouWant(product.payWhatYouWant || false);
                setIsPhysical(product.isPhysical || false);
                setConfirmations(product.confirmations || []);
                setFeatures(product.features || []);
                setRequirements(product.requirements || []);
                setLicenseStrategy(product.licenseStrategy || 'none');
                setLicenseKeysRaw(product.licenseKeys ? product.licenseKeys.join('\n') : '');
        ...
        <div className="relative">
            ...
            >
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <StatusBadge status={hideProduct ? 'hidden' : status} />
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {[
                                ...
                                { label: 'Status', value: hideProduct ? 'hidden' : status },
